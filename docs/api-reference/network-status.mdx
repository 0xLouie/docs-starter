...
description: Aggregate status for the NYC beta dataset.
openapi: "GET /api/v1/network/status"
---

Returns aggregate status for the New York City beta dataset.
...
| Field | Type | Notes |
|-------|------|-------|
| `status` | string | Always `operational` for the beta dataset |
...
<Note>
`activeNodes` and `avgLatency` are fixed values in the beta dataset, not live
